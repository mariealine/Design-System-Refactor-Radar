...
- **Design system** — Your shared colors, typography, spacing, and component patterns. ds-coverage helps you keep the codebase consistent with these decisions.
- **Violation** — A place in the code where a hardcoded value (e.g. `#ff0000` or `text-sm`) is used instead of a design token or convention. The scanner finds these so you can fix them.
- **Coverage** — The share of files (or lines) that have no violations. Higher coverage = more consistent use of your design system.
- **Dashboard** — A single HTML file generated by the scan. Open it in a browser to explore violations, components, and migration plans.
- **Config** — One file (e.g. `ds-coverage.config.js` or `ds-coverage.config.mjs`) defines what to scan, which patterns to check, and where your components live. Run `npx ds-coverage init` to create it.

**Suggested first run:** `npx ds-coverage init` (answer the wizard), then `npx ds-coverage --open` to scan and open the dashboard.
...
- `ds-coverage.config.mjs` — your config file with patterns matching your stack (or .js / .cjs depending on setup)
...
- **Metrics — App** — Always shown. App-wide compliance and violations.
- **Metrics — Design System** — Shown if component analysis is enabled.
- **Components — API Redesign** — Shown if component analysis is enabled.
- **Migration Plan** — Shown if migration is enabled and you have at least one component mapping.
- **Refactoring Roadmap** — Shown if roadmap is enabled.
...
The config file (e.g. `ds-coverage.config.js` or `ds-coverage.config.mjs`) controls everything. Run `npx ds-coverage init` to generate one, or create it manually:
