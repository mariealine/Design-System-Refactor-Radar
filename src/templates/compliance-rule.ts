 ...
 * based on the Design System Assistant config. This ensures the AI enforces the same
...
> **This rule is auto-generated by Design System Assistant.** It stays in sync with the scanner config so that the AI assistant enforces the same rules the scanner detects.
...
2. **Run the scanner**: \`npx design-system-assistant\` — open the dashboard to see violations
3. **Prioritize**:
...
6. **Re-scan**: Re-run \`npx design-system-assistant\` to verify improvements
`;
}
